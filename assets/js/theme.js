		...
		function () {
			...
			if (0 !== e.length || 0 !== t.length || 0 !== c.length) {
				...
				for (var m = 0; m < t.length; m++) {
					...
					if (t[m].classList.add("bar-chart-" + m), null != v)
						for (var g = JSON.parse(v), y = 0; y < g.colors.length; y++) s = "\n        .bar-chart-".concat(m, " .ct-series:nth-child(").concat(y + 1, ") .ct-bar {\n            stroke: ").concat(g.colors[y], " !important;\n          }\n        "), i.appendChild(document.createTextNode(s));
		...
		function () {
			...
			if (!(e.length <= 0))
				for (var t = 0; t < e.length; t++) e[t].querySelector('[data-bs-toggle="price"]').addEventListener("change", function (e) {
					...
					if (1 == r.checked)
						for (var c = a, s = e, i = 0; i < c.length; i++) c[i].classList.add("d-none"), s[i].classList.remove("d-none")
				})
		}(),
		function () {
